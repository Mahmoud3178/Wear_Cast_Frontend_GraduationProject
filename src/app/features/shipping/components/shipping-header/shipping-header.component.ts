...
@Input() undeliveredCount = 0;
  ...
  ngOnInit(): void {
    ...
    // Refresh from profile service to ensure accuracy
    this.profileSub = this.shippingCompanyService.getManager().subscribe({
      next: (manager) => {
        if (manager) {
          this.userName = `${manager.firstName} ${manager.lastName}`.trim();
        }
      }
    });
